    ...
    public function handle()
    {
        ...
        foreach ($activities as $activity) {
			...
			if (!is_null($activity_)){
				$cols = Col::where('Latitude','>=',$activity_->LatitudeMin - 1000)
					->where('Latitude','<=',$activity_->LatitudeMax + 1000)
					->where('Longitude','>=',$activity_->LongitudeMin - 1000)
					->where('Longitude','<=',$activity_->LongitudeMax + 1000)
            ...
            if ($process) { //new activity
				...
				if (is_null($activity->start_latlng) || is_null($activity->end_latlng)){
					continue;
				} else if (count($activity->start_latlng) != 2 || count($activity->end_latlng) != 2) {
					continue;
				}		
