...
class UsersController extends Controller
{
	...
	public function index(Request $request)
    {
		...
		$cols_most = Col::join('usercol','usercol.ColID', '=', 'cols.ColID')
			...
			->orderBy(DB::raw('count(usercol.id)'), 'DESC')
			->limit(5)
			->get(['cols.ColIDString', 'cols.Col', 'cols.Country1', 'cols.Country1IDString', 'cols.Country2', 'cols.Country2IDString', DB::raw('count(usercol.id) as users')]);
		...
		$following = Auth::user()->following()
			...
			->select('users.id', 'users.name', 'users.slug', 'usercol.ColID', 'usercol.ClimbedAt', 'cols.ColIDString', 'cols.Col', 'cols.Country1', 'cols.Country1IDString', 'cols.Country2', 'cols.Country2IDString')
		...
		foreach($countries as $country){
			$col = Col::join('usercol','usercol.ColID', '=', 'cols.ColID')
				...
				->groupBy('cols.ColID')
				->orderBy(DB::raw('count(usercol.id)'), 'DESC')
				->first(['cols.ColIDString', 'cols.Col', 'cols.Country1', 'cols.Country1IDString', 'cols.Country2', 'cols.Country2IDString', DB::raw('count(usercol.id) as users')]);
	...
	public function _search(Request $request)
    {
		$users = User::selectRaw("users.name, users.slug, useruser.id as following")
			...
			->whereRaw("users.name NOT LIKE '%http%'")
