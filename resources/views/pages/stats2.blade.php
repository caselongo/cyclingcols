...
?>
	...
	var getTopStats = function(){
		$.ajax({
			type: "GET",
			url : "/service/stats2/top/{{$country->URL}}",
			dataType : 'json',
...
@foreach ($stattypes as $stattype_)
				<a class="dropdown-item font-weight-light" href="/stats2/{{$stattype_->URL}}/{{$country->URL}}">
...
@foreach ($countries as $country_)
				<a class="dropdown-item font-weight-light" href="/stats2/{{$stattype->URL}}/{{$country_->URL}}">
...
?>
		...
		@if ($stattype->StatTypeID == 0)		
						<a href="/stats2/{{$stattype_current->URL}}/{{$country->URL}}"><i id="col-first-all" class="fas fas-grey fa-search-plus" title="show all"></i></a>
		@endif
					</div>
