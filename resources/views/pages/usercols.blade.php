...
@foreach ($sorttypes as $sorttype_)
				<a class="dropdown-item font-weight-light" href="/athlete/{{$user->slug}}/cols/{{$geourl}}/{{$sorttype_->URL}}">
...
@endforeach
		  ...
		  <li class="nav-item dropdown">
			...
			<div class="dropdown-menu">
@foreach ($regions as $region_)
				<a class="dropdown-item font-weight-light" href="/athlete/{{$user->slug}}/cols/{{$region_->URL}}/{{$sorttype->URL}}">
					{{$region_->Region}}
				</a>
...
@endforeach
		  ...
		  <li class="nav-item dropdown">
			...
			<div class="dropdown-menu">
@foreach ($subregions as $subregion_)
				<a class="dropdown-item 
	@if	($subregion_->SubRegionID > 0)		
				dropdown-item-inline 
	@endif
				font-weight-light" href="/athlete/{{$user->slug}}/cols/{{$subregion_->URL}}/{{$sorttype->URL}}">
...
@if (count($cols) == 0)
	@if ($subregion->SubRegionID > 0)
					<span class="text-small-75">No cols in {{$subregion->SubRegion}} climbed yet.</span>
	@elseif ($region->RegionID > 0)
					<span class="text-small-75">No cols in {{$region->Region}} climbed yet.</span>
	@else
...
@else
	...
	@foreach($cols as $col)
					<div class="align-items-end d-flex">
						<img src="/images/flags/{{$col->Country1IDString}}.gif" title="{{$col->Country1}}" data-toggle="tooltip" class="flag mr-1">
		@if ($col->Country2)
						<img src="/images/flags/{{$col->Country2IDString}}.gif" title="{{$col->Country2}}" data-toggle="tooltip" class="flag mr-1">
		@endif
					...
					<div class="card mb-3">
						...
						<div class="card-body p-0">
							<h6 class="card-title p-2 m-0 font-weight-light">
								<img src="/images/flags/{{$col->Country1IDString}}.gif" title="{{$col->Country1}}" data-toggle="tooltip" class="flag">
		@if ($col->Country2)
								<img src="/images/flags/{{$col->Country2IDString}}.gif" title="{{$col->Country2}}" data-toggle="tooltip" class="flag flag2">
		@endif
